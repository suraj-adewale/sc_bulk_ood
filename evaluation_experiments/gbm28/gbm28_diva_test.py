...
if __name__ == "__main__":
    ...
    parser.add_argument("-test", "--test_id",
                        dest="test_id",
                        help="ID of GBM28 experiment that was used in testing")
    parser.add_argument("-ref", "--reference_Y_id",
                        dest="reference_Y_id",
                        help="ID of GBM28 experiment to get appropriate Y order in model")
    args = parser.parse_args()

    # read in trained model 
    encoder = K.models.load_model(f"{args.res_data_path}/encoder")

    # read in the test / train data
    _, Y_train, _, _ = sc_preprocess.read_diva_files(args.aug_data_path, None, args.reference_Y_id)
    X_pbmc2, Y_pbmc2, pbmc2_gene_df, _ = sc_preprocess.read_diva_files(args.aug_data_path, None, args.test_id)
    ...
    pbmc2_gene_df = pbmc2_gene_df.to_frame(index=False, name='gene_ids')
    ...
    res_file = os.path.join(args.res_data_path, f"test-{args.test_id}-DIVA.pkl")
